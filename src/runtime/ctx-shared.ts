...
export class SharedRuntime {
  ...
  createCtxBase(screenshot: (label: string) => Promise<void>): CtxBase {
    const { validator, inputs, artifact, phase } = this.opts;
    return {
      entryUrl: artifact.manifest.entryUrl,
      emit: (schemaName, item) => {
        this.throwIfAborted();
        const result = validator.validate(schemaName, item);
        if (result.ok) {
      ...
      input: (name) => {
        const declared = artifact.manifest.inputs.find((i) => i.name === name);
