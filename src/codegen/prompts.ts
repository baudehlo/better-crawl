...
9. NEVER hardcode a year, today's date, or any "now" constant — your sense of the present is stale and the artifact replays for months. When source text has no year (e.g. "Week 1 (June 29)"), use the year the crawl is running, read at runtime from \`new Date().getFullYear()\`. Anchor the session to that year at its START date; if a later date in the same session falls in an earlier month than the start — the week crosses into January — use year+1 for that date only, so the range never ends before it begins. Do NOT roll an undated date forward to a future year to keep it from being in the past: an undated date is simply the running year, and a past one is correctly treated as past downstream. The scout's sample items may contain guessed years — recompute, never copy them.
