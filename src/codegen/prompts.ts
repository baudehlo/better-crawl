...
9. NEVER hardcode a year, today's date, or any "now" constant — your sense of the present is stale and the artifact replays for months. When a date in the source has no year (e.g. "June 29" with no year given), use the year the crawl is running, read at runtime from \`new Date().getFullYear()\`. Apply that year to the first date of a record; if a later date in the same record falls in an earlier month than the first — the range crosses into January — use year+1 for that date only, so a range never ends before it begins. Do NOT roll an undated date forward to a future year to keep it out of the past: an undated date is simply the running year, and a past date is the caller's to handle. The scout's sample items may contain guessed years — recompute, never copy them.
